"""`cloudops clone` — clone an existing instance: recreate its configuration
(type/image/settings) as a NEW instance. By default only the template is cloned;
pass --with-data to also replicate disk contents:
...
Creates a billed instance, so it follows the same contract as `cloudops spawn`:
...
def main(argv: "Optional[list[str]]" = None) -> int:
    parser = argparse.ArgumentParser(
        prog="cloudops clone",
        description="Clone an instance's configuration into a new instance")
    ...
    parser.add_argument("--open-port", type=int, action="append", dest="open_ports", metavar="PORT",
                        help="expose a TCP port on the clone (repeatable; see `cloudops spawn`)")
