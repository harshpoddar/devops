"""`cloudops stop` — stop a running instance without destroying it.
...
COST_NOTE = {
    ...
    "vast": ("GPU billing stopped; disk storage still bills until destroyed. Restart is not "
             "guaranteed — the host may rent these GPUs to someone else (`cloudops clone` can "
...
def main(argv: "Optional[list[str]]" = None) -> int:
    parser = argparse.ArgumentParser(
        prog="cloudops stop", description="Stop a running cloud instance (keeps its disk)")
